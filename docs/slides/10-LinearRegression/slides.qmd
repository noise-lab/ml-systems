...
In words: multiply each feature by its weight, add them up, and add a constant offset — a dot product between a weight vector and the feature vector. *(Equation in the appendix.)*
...
## Problem Framing: Where Do Inputs Come From?

**Given** a vector of inputs, **predict** a real-valued output. The inputs can come from many sources:

- **Quantitative inputs** — raw measurements (packet counts, RTTs)
- **Transformations** of quantitative inputs — log, square root, square
- **Basis expansions** — square, cube, etc. (previewed later this lecture)
- **Numeric coding** of qualitative inputs — e.g., protocol type as 0/1 columns
- **Interactions** between variables — e.g., $x_1 \cdot x_2$

The model stays linear *in the coefficients* regardless of where the inputs came from.

::: {.notes}
This framing matters: students often assume "linear regression" means the features must be raw measurements. In fact any derived quantity works — logs of heavy-tailed features (flow sizes!), one-hot encodings of protocols, and interaction terms. This foreshadows basis expansion in the second part of the lecture. See "Problem Framing" in the source slides.
:::
...
**Training** picks the line that minimizes the sum of squared residuals — for a linear model this has a closed-form solution, the *normal equation*. *(Derivation in the appendix.)*
...
For the least-squares fit (the normal equation — see the appendix) to give a *good* estimate:
...
*Which coefficients actually matter?* Z-scores and F-tests check whether a coefficient is significantly non-zero — useful for pruning irrelevant features. *(Formulas in the appendix.)*
...
In words: the prediction is a weighted sum of the *transformed* features rather than of the raw inputs. *(Equation in the appendix.)*
...
## Inductive Bias {.smaller}

Learning assumes some structure in the data we *have* seen (training) — and the data we *have not* (test).

Consider two functions built from the same training set:

- $f(x') = 1$ if some $x$ with label 1 matches $x'$ in the dataset, $0$ otherwise
- $g(x') = 0$ if some $x$ with label 0 matches $x'$ in the training set, $1$ otherwise

Both are perfectly consistent with the training data — yet they give **exactly opposite answers** on every datapoint *not* in the training set. Data alone cannot choose between them; the learner's **assumptions** must.

::: {.notes}
This is the classic argument for why learning is impossible without bias. Both f and g memorize the training data and agree on it completely, but disagree everywhere else. Any preference for one over the other is an assumption the algorithm brings, not something the data provides. See "Inductive Bias" in the source slides.
:::
...
*(Formal bias-variance decomposition in the appendix.)*
:::
:::
...
*(Full objective functions and the ridge closed-form solution are in the appendix. Standardize inputs before fitting either.)*
...
| $x_1, \ldots, x_N$ | input features; $\mathbf{x}$ is the feature vector |
| $w_1, \ldots, w_N$ | weights; $\mathbf{w}$ is the weight vector |
| $b$ | bias (intercept); absorbed into $\mathbf{w}$ when $x_0 = 1$ |
| $N$ | number of features |

## Basis Expansion {.smaller}
...
|---|---|
| $X$ | raw input(s) |
| $h_m(X)$ | the $m$-th basis function (e.g. $X^m$, a Gaussian bump, a Fourier term) |
| $\beta_m$ | coefficient on the $m$-th basis function — fitted by ordinary least squares |
