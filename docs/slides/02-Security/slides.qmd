...
::: {.notes}
The disinformation infrastructure work extends the DNS/registration approach. The key pedagogical point: the *content* of disinformation is hard to evaluate automatically (what is "false"?), but the *infrastructure* used to host and distribute it differs measurably from legitimate news infrastructure. Domain lifespan CDF (s32-i27.png) shows disinformation domains cluster below 10 years; news domains extend to 30+ years. The certificate SAN finding is counterintuitive — large SAN counts suggest shared certificate management infrastructure, which news organizations use, not disinformation operators.
:::
