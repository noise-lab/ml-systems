...
## Network-Level Features That Persist {.smaller}

Content and sender IPs change daily; *how* a spammer sends is much harder to change.

::: {.columns}
::: {.column width="50%"}
**Features visible from a single SMTP connection (no message body):**

- **Sending pattern:** number of recipient domains and messages per sender; message-size distribution and burstiness
- **AS / BGP route:** sender's AS, and whether the prefix is *ephemeral* (announced briefly, then withdrawn)
- **Geographic distance** between sender and recipient mail server
- **Spatiotemporal clustering:** distance to the *k* nearest senders in IP space, and whether those neighbours are also sending now

**Why these persist:** changing them costs the spammer real infrastructure, not just a new template.
:::
::: {.column width="50%"}
![Average numerical distance (IP space, log scale) to the *k* nearest senders. Spammers' neighbours are 1–2 orders of magnitude closer than legitimate senders' — many compromised hosts sit on the same network blocks.](images/snare-knn-ip-distance.png)
:::
:::

::: {.notes}
This is the "how, not what" idea made concrete. Each feature maps to one of the three invariants from the previous slide: ephemeral BGP routes → agility; message-size/recipient patterns and geographic distance → exposure; k-nearest-sender distance → coordination. The figure is from the original SNARE analysis: for each sender, compute the average IP-space distance to its k nearest neighbours that sent mail in the same window. Certain-spam senders sit roughly 10–30× closer to their neighbours than certain-ham senders at every k. Ask: which of these features could a spammer cheaply forge? (Geographic distance and AS are essentially unforgeable; message size and recipient count could be adjusted at the cost of throughput.)
:::

## SNARE: Combining Features {.smaller}

**Spatio-temporal Network-level Automatic Reputation Engine** — Hao, Feamster, Gray, Krasser, USENIX Security 2009

::: {.columns}
::: {.column width="55%"}
- Input: **only network-level features** from the previous slide, computed from one day of query logs at a large spam-filtering appliance provider
- Classifier: **RuleFit** — an ensemble of decision-tree rules combined in a sparse linear model, so the learned rules are inspectable
- Evaluated with 10-fold cross-validation
- Reputation can be assigned on a **single query**, before the message body is accepted

**Takeaway:** about 70% of spam caught with roughly 0.4% false positives, using no content at all — a cheap first-stage filter in front of content analysis.
:::
::: {.column width="45%"}
![SNARE confusion matrix: 70% of spam is classified as spam; 0.44% of legitimate mail is misclassified.](images/snare-rulefit-confusion.png)
:::
:::

::: {.notes}
Builds on Ramachandran, Feamster, Vempala, "Filtering Spam with Behavioral Blacklisting," ACM CCS 2007. RuleFit (Friedman and Popescu) fits many shallow decision trees, treats each root-to-leaf path as a binary rule, then fits an L1-regularised linear model over the rules plus the raw features; that is why the result is interpretable. The 70% / 0.44% numbers are the headline result from the paper's Table. Emphasise the cost argument: this decision is made at connection time from a handful of features, so it scales to a mail-appliance provider's full volume, and the 30% it misses goes on to the (expensive) content filter. Also worth noting the class imbalance: 0.44% false positives sounds tiny but is a lot of legitimate mail at scale, which is why SNARE was framed as a reputation signal rather than a hard block.
:::

## Going Back Earlier in Time {.smaller}

Each step earlier in the attack lifecycle gives cheaper, more robust detection — and a bigger head start.

| When we detect | What we see | Example |
|---|---|---|
| **Message time** | Content, URLs, attachments | Bayesian / content filters |
| **Connection time** | Sender IP, AS, route, timing, neighbours | SNARE (previous slide) |
| **Domain-registration time** | Registrar, nameservers, early DNS lookups | Next slides |

- Attackers use **URLs and domains** to propagate scams and malware; those domains must be registered, and DNS records established, *before* any message is sent
- **Problem:** reputation for a brand-new domain? Prior work waited to observe record changes, which takes time
- **Alternative:** look at query and registration patterns within a short window after registration

::: {.notes}
This is the bridge to the DNS material. The theme of the whole section is moving detection left: content → connection → registration. Ask students what the attacker has to commit to at each stage and how expensive it is to change (a template: free; a bot's IP: cheap; a registered domain with nameservers: costs money and leaves a paper trail). The next two slides show what "early DNS behaviour" looks like empirically (Hao, Feamster, Pandrangi, IMC 2011).
:::
...
**Fast Gradient Sign Method (FGSM):** perturb each input feature a tiny step in the direction that increases the loss.

Take the gradient of the loss w.r.t. the input; step in the direction that increases loss. *(Equation in the appendix.)*
...
Train on the worst-case perturbation within a small budget. *(Equation in the appendix.)*
...
**FGSM** (Goodfellow et al., 2015) builds an adversarial example in one step:

$$x_{\text{adv}} = x + \epsilon \cdot \text{sign}(\nabla_x L(\theta, x, y))$$

- $x, y$ — a clean input and its true label
- $\theta$ — the (known, white-box) model parameters
- $L(\theta, x, y)$ — the training loss; $\nabla_x L$ is its gradient with respect to the *input*, not the weights
- $\text{sign}(\cdot)$ — element-wise sign, so every feature moves by exactly $\pm\epsilon$
- $\epsilon$ — the perturbation budget (an $\ell_\infty$ bound)

**PGD** iterates this with a smaller step $\alpha$ and projects back onto the $\epsilon$-ball after each step:
$x^{(k+1)} = \Pi_{\|x' - x\|_\infty \le \epsilon}\!\left(x^{(k)} + \alpha \cdot \text{sign}(\nabla_x L(\theta, x^{(k)}, y))\right)$.

::: {.notes}
Companion to "Adversarial Evasion: Gradient-Based Attacks." The sign function means every feature gets perturbed by exactly $\pm\epsilon$, making the attack cheap to compute — one backward pass. PGD is just iterated FGSM with a projection step and generally finds stronger adversarial examples. For networking applications, remind students of the problem-space constraint: FGSM may suggest setting "mean packet size" to a fractional value, which is meaningless in a real network.
:::

## Adversarial Training Objective

**Adversarial training** (Madry et al., 2018) replaces the usual loss with a min–max objective:

$$\min_\theta \; \mathbb{E}_{(x,y)} \left[ \max_{\|\delta\| \leq \epsilon} L(\theta, x + \delta, y) \right]$$

- **Inner max** — the attacker: find the perturbation $\delta$ within budget $\epsilon$ that hurts most (approximated with PGD)
- **Outer min** — the defender: choose parameters $\theta$ that minimize loss on those worst-case inputs
- $\mathbb{E}_{(x,y)}$ — averaged over the training distribution

In practice: each mini-batch is attacked with PGD, and the model is updated on the attacked batch with the correct labels.

::: {.notes}
Companion to "Defenses: Raising the Cost of Attacks." The min-max formulation is the clearest way to state what "robust model" means: you want a model that minimizes loss even against the worst-case perturbation within the allowed budget $\epsilon$. Robustness is only guaranteed within that budget and that norm — an attacker who can move outside the $\epsilon$-ball, or who perturbs in problem space rather than feature space, is not covered. For networking, adversarial examples must also satisfy protocol constraints, not just feature-space bounds.
:::
