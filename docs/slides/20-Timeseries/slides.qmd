...
A series is a trend plus a seasonal pattern plus a residual; differencing
removes the trend. *(Equation in the appendix.)*
...
**ARMA(p, q)** = an **AR** part (weighted sum of the past $p$ values) plus an **MA** part (weighted sum of the past $q$ shocks). *(AR, MA, and ARMA equations in the appendix.)*
...
The additive decomposition behind "Making a Series Stationary":
...
::: {.notes}
Seasonal decomposition is the diagnostic tool that precedes fitting a SARIMA model (the book's Seasonal ARIMA discussion). Decomposing first reveals which components actually need to be modeled, preventing you from fitting a trend into the AR terms. Differencing (the "I" in ARIMA) is the alternative when only the trend needs removing.
:::
