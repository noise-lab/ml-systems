...
::: {.columns}
::: {.column width="50%"}
The **sigmoid** $\sigma(z)$ maps any real-valued score $z$ — here the linear score $\mathbf{w}^T\mathbf{x}$ — to a number between 0 and 1. *(Equation in the appendix.)*
...
The **logistic regression model** is then: the probability of class 1 is the sigmoid of the linear score. *(Equation in the appendix — along with why this is the maximum-likelihood model for Bernoulli labels.)*
:::
::: {.column width="50%"}
![The logistic (sigmoid) function: maps any real-valued score to a probability in $[0,1]$, crossing $0.5$ at $z=0$](images/s05-i01.png)
:::
:::
...
The model: the probability that a packet is a response is the sigmoid of a weighted packet size plus an offset. *(Equation in the appendix.)*
...
See the book's appendix activity for a hands-on implementation.
:::
:::
...
*(Cross-entropy loss and gradient descent details in the appendix.)*
:::
::: {.column width="50%"}
...
Each class gets its own weight vector; the probability of class $k$ is its exponentiated score divided by the sum of every class's exponentiated score. *(Equation in the appendix.)*
...
## The Sigmoid and the Logistic Model {.smaller}
...
**DNS instance** (one feature, packet size):

$$P(\text{response} \mid \text{size}) = \sigma(w \cdot \text{size} + b)$$

| Symbol | Meaning |
|---|---|
| $\mathbf{x}$, $\mathbf{w}$ | feature vector; weight vector |
| $z = \mathbf{w}^T\mathbf{x}$ | linear score (the log-odds) |
| $\sigma(z)$ | sigmoid (logistic) function, valued in $(0, 1)$ |
| $y \in \{0, 1\}$ | binary label |
| $w$, $b$ | weight on packet size and bias in the one-feature DNS model |
...
| $\mathbf{w}_k^T\mathbf{x}$ | linear score for class $k$ |
| denominator | normalizer, so the $K$ probabilities sum to 1 |
...
**Alternative — Newton's method:** Taylor-expand the loss around the current
weights, take the gradient, set to zero. Second-order → faster convergence,
but requires the Hessian.
