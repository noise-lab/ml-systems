...
**Convergence criterion:** no re-assignments, or SSE below threshold — the goal is to
minimize the total squared distance from each point to its centroid. *(Equation in the appendix.)*
...
**DBSCAN** = **D**ensity-**B**ased **S**patial **C**lustering of **A**pplications with **N**oise (Ester et al., KDD 1996)
...
The quantity k-means minimizes (see "How K-Means Works"), the **sum of squared errors**:

$$\text{SSE} = \sum_{j=1}^{K} \sum_{x \in C_j} \text{dist}(x,\, m_j)^2$$

- $K$: number of clusters; $C_j$: the set of points assigned to cluster $j$
- $m_j$: the centroid (mean) of cluster $j$
- Each E-step and M-step can only lower SSE, so the algorithm always converges

::: {.notes}
The "means" in K-means literally refers to the M-step — moving each centroid
to the arithmetic mean of its cluster, which is the choice of $m_j$ that
minimizes SSE for a fixed assignment. The algorithm always converges (SSE is
non-increasing) but may reach a local minimum. See book Chapter 6, K-Means
section. Ask: "What happens to SSE as K increases?"
:::
